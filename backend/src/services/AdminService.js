...
class AdminService {
  ...
  async generateDB() {
    const { stdout, stderr } = await execPromise(
      "cd ../medropax_vrp3d && python rebuild_database.py"
    );
  ...
  async generateOrders() {
    const { stdout, stderr } = await execPromise(
      "cd ../medropax_vrp3d && generate_orders.py "
    );
  ...
  async generateVehicles() {
    const { stdout, stderr } = await execPromise(
      "cd ../medropax_vrp3d && python generate_vehicles.py"
    );
  ...
  async generateDelivers() {
    const { stdout, stderr } = await execPromise(
      "cd ../medropax_vrp3d && python deliver_orders.py"
    );
